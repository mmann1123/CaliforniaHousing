...
# working workspace  load(paste('G:\\Faculty\\Mann\\Share\\Final_Variables_2\\CountyRun\\JepsonGroups6\\JepsonAreaSpecCity6.7TUrbNS+D20km','workspace.RData',sep='\\'))




##########################################################################################
VERSION = "6.9ppnoneTUrbNS+D20km"  # final model 6.9TUrbNS+D20km scenario D
DIRECTORY = paste("G:\\Faculty\\Mann\\Share\\Final_Variables_2\\CountyRun\\JepsonGroups6\\JepsonAreaSpecCity",VERSION,sep="") 

load(paste('G:\\Faculty\\Mann\\Share\\Final_Variables_2\\CountyRun\\JepsonGroups6\\JepsonAreaSpecCity6.9ppnoneTUrbNS+D20km','workspace.RData',sep='\\'))
...
specifyit <- function(counties ){
    ...
    for (urban1 in 1:2){
        ...
        resultlist <<- specifierAIC5(county=counties,urban = urban1)    # originally used specifierAIC4 which includes ppall
        print(resultlist)
...
load( file = paste(DIRECTORY,"regression_out.RData",sep="\\") )
    ...
    # must have splm installed
    urban_other_coefficients_table =  as.data.frame( t(urban_other_coefficients[[1]]),) # start a dataframe and fill it with coefficients
    ...
    # calc median coefficient values
    urb_med_c_other =  apply(urban_other_coefficients_table,2,median,na.rm=T)
    # Calc pvalues
    ...
    #calc median pvalues
    urb_med_p_other = apply(urban_other_P_table,2,median,na.rm=T)
    # calc percent .1 > p
    urb_med_pc_other = apply(urban_other_P_table<=0.1,2,sum,na.rm=T)/apply(!is.na(urban_other_P_table),2,sum,na.rm=T) # percentage significant
    library(xtable)     
    urb_table_out = data.frame( EstimateMedian=urb_med_c_other, PValueMedian=urb_med_p_other, PCSignificant=urb_med_pc_other*100)
    urb_table_out2 = xtable(urb_table_out) 
    digits(urb_table_out2)=3
    print(urb_table_out2,floating=T)
  #> colnames(mat) <- c("$R^2$", "$\\bar{R}^2$", "F-stat", "S.E.E", "DW")
    ...
    rur_med_c_other = apply(rural_other_coefficients_table,2,median,na.rm=T)
    ...
    rur_med_p_other=apply(rural_other_P_table,2,median,na.rm=T)
    rur_med_pc_other=apply(rural_other_P_table<=0.1,2,sum,na.rm=T)/apply(!is.na(rural_other_P_table),2,sum,na.rm=T) # percentage significant
    rur_table_out = data.frame( EstimateMedian=rur_med_c_other, PValueMedian=rur_med_p_other, PCSignificant=rur_med_pc_other*100)
    rur_table_out2 = xtable(rur_table_out)                                  
    print(rur_table_out2,floating=T)

    urb_rur_table_out = rbind(rur_table_out,urb_table_out)                                  
    urb_rur_table_out2 = xtable(urb_rur_table_out) 
    digits(urb_rur_table_out2)=c(0,-3,3,3)
    print(urb_rur_table_out2,floating=T, type = "html", file='G://Faculty//Mann//Share//CA Housing_writeup//Submitted to Land Use Policy//Second Submission//Graphs//regression_table.html' )                              
...
do.call(rbind.fill, list(urban_other_table2[[1]],urban_other_table2[[2]]))
    ...
    median(rhop_rural_regressions,na.rm=T)
    ...
    median(c(rsqr_urban_regressions ))
    median(c( rsqr_rural_regressions))
    range(c(rsqr_urban_regressions,rsqr_rural_regressions))

#######################################################
# measure of fit 

resid =  lapply( 1:length(regressionlist), function(x) regressionlist[[x]]$residuals)   
ys =  lapply( 1:length(regressionlist), function(x) regressionlist[[x]]$model$y)   
reg_RMSE = lapply( 1:length(regressionlist), function(x) sum((regressionlist[[x]]$residuals)^2)/length(regressionlist[[x]]$residuals))  

# this is for all data points not by regression
storeit = data.frame( ys=unlist(ys),res=unlist(resid) , prc=unlist(resid)/unlist(ys)*100,class= cut(unlist(ys),breaks=c(0.01,.5,1,1.5,2,2.5,3,4,5,10,20,30)))

windows()
ylim1 = boxplot.stats(storeit$prc)$stats[c(1, 5)]
ggplot(na.omit(storeit), aes(factor(class), prc))+ geom_boxplot(outlier.shape = NA)+ coord_cartesian(ylim = ylim1*1.05)+xlab('Density Class')+ylab('Percent Estimate Error')
 
windows()
ylim1 = boxplot.stats(storeit$res)$stats[c(1, 5)]
ggplot(na.omit(storeit), aes(factor(class), res))+ geom_boxplot(outlier.shape = NA)+ coord_cartesian(ylim = c(-4,8))+xlab('Density Class')+ylab('Residual')

windows()
ggplot(na.omit(storeit), aes(factor(class), reg_RMSE))+ geom_boxplot(outlier.shape = NA)+ coord_cartesian(ylim = c(-4,8))+xlab('Density Class')+ylab('Residual')


# extract ids
for(x in 1:length(regressionlist)){
    regressionlist[[x]]$model$residual = regressionlist[[x]]$residuals
    regressionlist[[x]]$model$nametime = row.names(regressionlist[[x]]$model)# add row id

   for(y in 1:dim(regressionlist[[x]]$model)[1]){  #split name
    regressionlist[[x]]$model$name[y]=  strsplit(regressionlist[[x]]$model$nametime,'-')[[y]][1]  
    regressionlist[[x]]$model$time[y]=  strsplit(regressionlist[[x]]$model$nametime,'-')[[y]][2] }
  }
# calculate RMSE for each SBG
head(regressionlist[[x]]$model)
rmseer= function(x){sum(x^2)/6}
for(x in 1:length(regressionlist)){  
  a= aggregate(data=regressionlist[[x]]$model,residual ~ name,FUN=rmseer )
  names(a)= c('name','RMSE')
  regressionlist[[x]]$model=merge(regressionlist[[x]]$model,a,by='name')
  regressionlist[[x]]$model$reg_num = x  # to avoid having many regressions with same name
}

windows()
SBG_RMSE = lapply( 1:length(regressionlist), function(x) regressionlist[[x]]$model$RMSE )  
year = lapply( 1:length(regressionlist), function(x) regressionlist[[x]]$model$time )
name = lapply( 1:length(regressionlist), function(x) paste(regressionlist[[x]]$model$reg_num,regressionlist[[x]]$model$name,sep='_') )
 

storeit = data.frame(year=unlist(year),name=unlist(name),SBG_RMSE= unlist(SBG_RMSE),ys=unlist(ys),res=unlist(resid) , prc=unlist(resid)/unlist(ys)*100,class= cut(unlist(ys),breaks=c(0.01,.5,1,1.5,2,2.5,3,4,5,10,20,30)))
storeit$year = as.numeric(as.character(storeit$year))

ggplot(na.omit(storeit ), aes(factor(class), SBG_RMSE))+ geom_boxplot(outlier.shape = NA)+ coord_cartesian(ylim = c(-4,8))+xlab('Density Class')+ylab('RMSE by Split Block Group')

# look at error by growth path classes
library(plm)
storeit_p = pdata.frame(storeit,index=c('name','year'))
head(storeit_p)
storeit_p$ys_lag = lag(storeit_p$ys, 5)

storeit_p2 = na.omit(storeit_p)
head(storeit_p2)
storeit_p2$class= cut(storeit_p2$ys_lag,breaks=c(0,0.01,.5,1,1.5,2,2.5,3,4,5,10,20,30),right=F)
windows()
ggplot(na.omit(storeit_p2 ), aes(factor(class), SBG_RMSE))+ geom_boxplot(outlier.shape = NA)+ coord_cartesian(ylim = c(-1,4))+xlab('Density Growth Class: 1950-2000')+ylab('RMSE by Split Block Group')



# actual = urban_regressions[[2]]$residualsXX + urban_regressions[[2]]$fitted.valuesXX
# ybar = mean(actual)
# sstot = sum((actual-ybar)^2)
# ssres = sum((urban_regressions[[2]]$residualsXX)^2)
# 
# r2 = 1-(ssres/sstot)
# r2
# summary(urban_regressions[[2]])$rsqr
# summary(urban_regressions[[1]]$residualsXX /urban_regressions[[1]]$fitted.valuesXX)
...
###################################################
#  get regression summaries 
#  predict out of sample without population forecasts use outofsample6loop_insampleprediction
  load( file = paste(DIRECTORY,"regression_out.RData",sep="\\") )
