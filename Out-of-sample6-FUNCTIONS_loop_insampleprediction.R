...
# this version creates in sample predictions for years 1990 and 2000 for linear, non-linear, and non-linear with population normalization
# NOTE: can't use actual population stats from the finance bearue need to back cast them. 
# compare using mean sq prediction errorf
# not used in final project  use region rural growth
...
########################################################################################
# estimate and estimate in sample error 
...
spec_holder$formulas = 'HuDac4 ~COUNTY+tminave+I(tminave^2)+tmaxave+I(tmaxave^2)+pptave+dist_ppall+HuDaclag'

sink(paste(DIRECTORY,"\\specificationsAIC_jepAREAcity_ISP2",VERSION,"_sink.txt",sep=""),append=F,split=T )
...
# RUN SEPERATELY ONCE FOR WITH TIME DUMMY (AND POP CORRECTION) AND ONCE WITHOUT
runit <- function(counties){
      ...
      for (urban1 in  1:2){       
        ...
        RMSE_out =  estimate_export_region_ISP(urban=urban1,counties=counties,spec_fn=spec_fn,no_time=T)  
        # should alternate uncorrected, corrected RMSE
        RMSE_list_out = c(RMSE_list_out,RMSE_out)
          
        print("#################  ONE COUNTY COMPLETE ###############")  
        print(Sys.time())   
        
        }
      RMSE_list_out<<-RMSE_list_out
      return(RMSE_list_out )
}

system.time( lapply( jepFIDS[1:(length(jepFIDS)-0)  ], function(x) runit(unlist(x))) )

save(RMSE_list_out,  file = paste(DIRECTORY,"RMSE_list_out_nonlinear_noTimedummy.RData",sep="\\") ) 


#extract data from RMSE_list_out
linear_corr = load(paste(DIRECTORY,"RMSE_list_out_nonlinear_noTimedummy.RData",sep="\\"))



# uncorrected = data.frame(RMSE = unlist(lapply(seq(1,length(RMSE_list_out),by=2), function(x) unlist(RMSE_list_out[x]))),Type='Uncorrected')
# corrected = data.frame(RMSE =unlist(lapply(seq(2,length(RMSE_list_out),by=2), function(x) unlist(RMSE_list_out[x]))),Type='Corrected')
# data = rbind(uncorrected,corrected)

#write data to file 
data = unlist(RMSE_list_out)
write.table(data,sep=',',file='G:/Faculty/Mann/Share/Final_Variables_2/CountyRun/JepsonGroups6/JepsonAreaSpecCity6.9TUrbNS+D20km_ISP/linear_notime.csv')



library(ggplot2)
ggplot(data, aes(factor(Type),RMSE) )+geom_boxplot()

data_linear_notime  = data.frame(RMSE=(read.table('G:/Faculty/Mann/Share/Final_Variables_2/CountyRun/JepsonGroups6/JepsonAreaSpecCity6.9TUrbNS+D20km_ISP/linear_notime.csv',sep=',')),Type=rep('linear',113))
#data_linear_notime$x[data_linear_notime$x==max(data_linear_notime$x)]=NA   # removes maximum value
data_nonlinear_notime  = data.frame(RMSE=read.table('G:/Faculty/Mann/Share/Final_Variables_2/CountyRun/JepsonGroups6/JepsonAreaSpecCity6.9TUrbNS+D20km_ISP/nonlinear_notime.csv',sep=','),Type=rep('Uncorrected',113))
#data_nonlinear_notime$x[data_nonlinear_notime$x==max(data_nonlinear_notime$x)]=NA
data_linear_time  = data.frame(RMSE=read.table('G:/Faculty/Mann/Share/Final_Variables_2/CountyRun/JepsonGroups6/JepsonAreaSpecCity6.9TUrbNS+D20km_ISP/linear_time.csv',sep=','),Type=rep('linear_dummy',113))
#data_linear_time$x[data_linear_time$x==max(data_linear_time$x)]=NA
data_nonlinear_time  = data.frame(RMSE=read.table('G:/Faculty/Mann/Share/Final_Variables_2/CountyRun/JepsonGroups6/JepsonAreaSpecCity6.9TUrbNS+D20km_ISP/nonlinear_time.csv',sep=','),Type=rep('Corrected',113))
#data_nonlinear_time$x[data_nonlinear_time$x==max(data_nonlinear_time$x)]=NA

all_data = data.frame(rbind(data_linear_notime,data_nonlinear_notime,data_linear_time,data_nonlinear_time))
aggregate(x ~ Type  , data = all_data, mean)
aggregate(x ~ Type  , data = all_data, median)

windows()
ggplot(all_data, aes(factor(Type),x) )+geom_boxplot()#+ggtitle('Linear Specification')
windows()
ggplot(all_data[all_data$Type %in% c('Uncorrected','Corrected'),], aes(factor(Type),x) )+geom_violin()+ylab('RMSE') +xlab('Estimation Type') +stat_summary(fun.y=median, geom="point", fill="white", shape=21, size=4, position=position_dodge(0.9))

windows()
ggplot(data_nonlinear, aes(factor(Type),RMSE) )+geom_boxplot()+ggtitle('NonLinear Specification')
...
###################################################
#  get regression summaries 
#  predict out of sample without population forecasts
load( file = paste(DIRECTORY,"regression_out.RData",sep="\\") )

regressionlist = plm_out12  # get all regresions

model.data = regressionlist[1][[1]]$model
...
model.data$HuDaclag = new.den.range
model.data$HuDaclagSqr = new.den.range^2
model.data$HuDaclagQb = new.den.range^3

stats::predict(regressionlist[1][[1]],newdata)
...
##########################################################################
  ...
  estimate_export_region_ISP <- function(urban,counties,spec_fn,no_time){   
    ...
    if(no_time ==T ){fm3 = as.formula(paste( spec_fn,sep="") )}
    ...
    if(no_time ==T ){timedummy=F}
    if(no_time ==F ){timedummy=T}
    for(i in 1:length(counties)){
      if( dim(panel.data2XX[unfactor(panel.data2XX$COUNTY) %in% counties[i],])[1]>0 ){   #avoid empty counties (ie no rural or urban)
        RMSE = forecastTDPOP_County3_ISP(data=panel.data2XX[unfactor(panel.data2XX$COUNTY) %in% counties[i],], results=pool,urban=urban,Rural_threshold=urban_rural_threshold, Rural_Amp_factor=1,depreciation=1, county_scale=F, polys=Block.39_spC[unfactor(Block.39_spC$COUNTY) %in% counties[i],],formula=fm3, constant=T,timedummy=timedummy, years=0, calcA=T)
        RMSE_holder = c(RMSE_holder,RMSE) 
  ...
  forecastTDPOP_County3_ISP <- function(data,polys,urban,county_scale,Rural_threshold,Rural_Amp_factor,formula,results,depreciation,constant, years, timedummy,calcA){ 
    #############  forecast function for in-sample prediction error tests 
    ...
    fore.data = data[unfactor(data$time) %in% max(unfactor(data$time)),]      # isolate prediction year
    Xs  = model.matrix.lm(formula,fore.data, na.action=NULL)
  
    
    # if time109 exists, remove
    Xs = Xs[, !colnames(Xs) %in% c('time49','time109')]
...
#     
        ids2 = unique(data$SUM090)
     
        
        data_ider = polys[polys$SUM090 %in% ids2,]    
...
#     results$coefficients=  c(results$coefficients,start) # add initial iterate later NOT NEEDED NOT SINCE IN SAMPLE
    ...
    if(timedummy==F){
      # calculate in sample RSE before housing correction 
      ...
      data.hold[data.hold$time %in% max(unfactor(data.hold$time)),c("EVHudac4")] = NA     # NA out forecast data
      data.hold[is.na(data.hold$EVHudac4),c("EVHudac4") ]= Yfore[na.omit(match(row.names(data.hold),row.names(Yfore))),"Yfore"]   # replace with forecast
      
      data.hold$SqEr =(data.hold[data.hold$time==99,c("HuDac4")]-data.hold[data.hold$time==99,c("EVHudac4")])^2
      uncorrected_RMSE = sqrt(sum(data.hold$SqEr))
      
      RMSE = sqrt(sum(data.hold$SqEr))
      remove(data.hold)
    }
    ...
    if(timedummy ==T){
      ...
      # INSERT ESTIMATES INTO ORIGINAL DATA
      data[data$time %in% max(unfactor(data$time)),c("EVHudac4")] = NA     # NA out forecast data
      data[is.na(data$EVHudac4),c("EVHudac4") ]= Yfore[na.omit(match(row.names(data),row.names(Yfore))),"Yfore"]   # replace with forecast
      
      
      # calculate in sample RSE after housing correction 
      data$SqEr =(data[data$time==99,c("HuDac4")]-data[data$time==99,c("EVHudac4")])^2
      corrected_RMSE = sqrt(sum(data$SqEr))
      RMSE = sqrt(sum(data$SqEr))
    }
    
    #print(paste('uncorrected:',uncorrected_RMSE,'corrected:',corrected_RMSE))
    #return( list(uncorrected_RMSE,corrected_RMSE) )
    return(list(RMSE))   #if running seperately
